function plot_recon_com(recon, para, clim_z, figi, roi_name, savePath, varargin)
% plot spatial reconstruction maps for all orientation conditions by
% rotating them to centered at zero.
...
%% rotate all orientation conditions to centered at 45deg
% to avoid fitting inf slope for vertical line. After fitting the line, 
% I rotate the line to be centered at vertical line.

% initialize variables
img_all = [];
img_mean = [];
% img_*_fit is used for fitting the line
img_all_fit = [];
img_mean_fit = [];
for e = 1:n_epochs
    for m = 1:n_moduls
        for jj = 1: n_conds
            ...
            imgsz = [size(img_j,2), size(img_j,1)];
            ...
            for x = 1:imgsz(1)
                for y = 1:imgsz(2)
                    ...
                    if distance > img_radius
                        img_j(x, y) = NaN; % Set pixel value to NaN for outside the circle
                    end
                end
            end
            
            %% rotate each img based on the target orientation
            % rotate the image to be centered at vertical line
            img_rotated = imrotate(img_j, oris(jj), 'crop');
            img_rotated(img_rotated == 0) = NaN;
            % rotate to be centered at 45deg for line fitting! 
            % Not vertical line to avoid fitting inf slope!
            img_fit_rotated = imrotate(img_j, oris(jj)+45, 'crop');
            img_fit_rotated(img_fit_rotated == 0) = NaN;

            % add together
            if isempty(img_all)
                img_all = img_rotated;
                img_all_fit = img_fit_rotated;
            else
                img_all = cat(3, img_all, img_rotated);
                img_all_fit = cat(3, img_all_fit, img_fit_rotated);
            end
        end % of cyclying over conditions
        % average across three conditions
        if isempty(img_mean)
            img_mean = mean(img_all(:,:,(3*m-2):3*m), 3);
            img_mean_fit = mean(img_all_fit(:,:,(3*m-2):3*m), 3);
        else
            img_mean = cat(3, img_mean, mean(img_all(:,:,(3*m-2):3*m), 3));
            img_mean_fit = cat(3, img_mean_fit, mean(img_all_fit(:,:,(3*m-2):3*m), 3));
        end
    end % of cyclying over moduls
end % of cyclying over epochs 
...
for e = 1:n_epochs
    for m = 1:n_moduls
        ...
        % find current img
        img = img_mean(:,:,m);
        img_fit = img_mean_fit(:, :, m);

        % find current subplot
        map_ind = (e-1)*n_moduls + m;
        f = subplot(n_epochs*n_moduls, 1, map_ind);
        hold on
...
%         colorbar; 
        ...
        if ismember( 'fitline_all', varargin)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            imgsz = [size(img_fit,2), size(img_fit,1)];
            % find the image size
            dat = reshape(img_fit, [imgsz(1)*imgsz(2) 1]);
            ...
            [r, c] = ind2sub(size(img_fit), ind);
            % find the corresponding row and column of inds
            % row --> y-axis, column --> x-axis
            
            % plot selected voxels for sanity check
%             img_mask = zeros(imgsz(1), imgsz(2));
%             img_mask(ind) = img(ind);
%             imagesc(img_mask,clim_z); colormap magma; hold on;
            ...
            angle = atan(sl);
            % add 45 deg counterclockwise to the angle
            angle = deg2rad(rad2deg(angle) + 45);
...
%             f.Title.FontSize = 14;
        ...
        elseif ismember( 'fitline_stim', varargin)
            ...
            diff = round([size(img_fit,2), size(img_fit,1)]./2) - radius;
            %                 diff = diff - 10;
            % find the inds for pixels inside the stimulus-square
            image_stimSquare = img_fit(1+diff(1):diff(1)+2*radius, 1+diff(2):diff(2)+2*radius);
            ...
            % plot selected voxels for sanity check
%             img_mask = zeros(imgsz_stimSquare(1), imgsz_stimSquare(2));
%             img_mask(ind) = img(ind);
%             imagesc(img_mask,clim_z); colormap magma; hold on;
...
% suptitle(roi_name, 'FontSize', 30, 'FontWeight', 'bold');
legend('off');
