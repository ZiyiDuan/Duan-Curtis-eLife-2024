function [recon] = do_recon_real(subj,patname,regsname,selname,pRFparams,voxel_inds,sigmaOpt,para)
...
% weights for pRF parameters
% z-scoring the data for each run
subj = zscore_runs(subj,patname,selname);
patname_z = [patname, '_z'];
real = get_mat(subj,'pattern',patname_z); % (n_voxels, n_trials)
% Load the regressors
regressors = get_mat(subj,'regressors',regsname); % (n_conds, n_trials)
[n_conds, n_trials] = size(regressors);
% average weights for each condition
real_avg = (real*regressors')./(n_trials/n_conds); % (n_voxels, n_conds)
% select voxels based on voxind
real_avg = real_avg(voxind, :);
...
for i = 1:n_conds
    ...
    if ismember('lim', sigmaOpt)
        r = arrayfun(@(v) {real_avg(v,i).*...
            gauss2d(xgauss,ygauss,sigmadat(v,1),[pRFparams_ROI(tmp(v),4:5)])}, ...
            1:numel(tmp));  % (1, n_voxel) cell, each cell contain each voxel's map (n_pix, n_pix)
    ...
    rcat = cat(3, r{:}); % (n_pix, n_pix, n_voxel)
    % sum across voxel dimension
    recon(:,:,i) = sum(rcat, 3);
end

disp(['---- completed ', subj.header.id, ' ----']);
